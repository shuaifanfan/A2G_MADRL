...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        elif cmd == 'save_trajectory':
            remote.send(env.save_trajectory_from_outside(data))
    
        elif cmd == 'get_obs_from_outside':
            remote.send(env.get_obs_from_outside())
        elif cmd == 'continue_step':
            ob, reward, done, info = env.continue_step()
            remote.send((ob, reward, done, info))
        elif cmd == 'poi_get_obs_from_outside':
            remote.send(env.poi_get_obs())
        elif cmd == 'poi_step':
            ob, reward, done, info = env.poi_step(data)
            remote.send((ob, reward, done, info))
        else:
            raise NotImplementedError
...
class SubprocVecEnv(ShareVecEnv):
    ...
    def __init__(self, env_fns, spaces=None):
        ...
        ShareVecEnv.__init__(self, len(env_fns), observation_space,
                             share_observation_space, action_space)
    def poi_continue_step(self):
        for remote in self.remotes:
            remote.send(('continue_step',None))
        results = [remote.recv() for remote in self.remotes]
        obs, rewards, dones, infos = zip(*results)
        obs, rewards, dones, infos = listd_to_dlist(obs),listd_to_dlist(rewards),dones,infos
        for key in obs.keys():
            obs[key] = np.stack(obs[key])
        for key in rewards.keys():
            rewards[key] = np.stack(rewards[key])
        return obs,rewards,dones, infos
    
    
    def poi_step(self,actions):
        self.step_async(actions,poi=True)
        return self.step_wait(poi=True)
    
    
    def step_async(self, actions,poi=False):
        if poi:
            for remote, action in zip(self.remotes, actions):
                remote.send(('poi_step', action))  # action作为参数发给各个环境线程
        else:
            actions = dlist_to_listd(actions)
            for remote, action in zip(self.remotes, actions):
                remote.send(('step', action))  # action作为参数发给各个环境线程
        self.waiting = True

    def step_wait(self,poi=False):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        obs, rewards, dones, infos = zip(*results)
        
        if poi:
            obs, rewards, dones, infos = listd_to_dlist(obs),rewards,dones,infos
            for key in obs.keys():
                obs[key] = np.stack(obs[key])
        else:
            obs, rewards, dones, infos = listd_to_dlist(obs),listd_to_dlist(rewards),dones,infos
    ...
    def save_trajectory(self,info):
        for remote in self.remotes:
            remote.send(('save_trajectory', info))
        results = [remote.recv() for remote in self.remotes]
        return results[0]
    ...
    def poi_get_obs_from_outside(self):
        for remote in self.remotes:
            remote.send(('poi_get_obs_from_outside', None))
        obs = [remote.recv() for remote in self.remotes]
        obs = listd_to_dlist(obs)
        for key in obs.keys():
            obs[key] = np.stack(obs[key])
