...
def getArgs(env, input_args=None):
    ...
    agent_args.n_node = len(env.nx_g.nodes())
    ...
    agent_args.n_minibatch = 5
    agent_args.batch_size = input_args.batch_size
    ...
    agent_args.observation_dim = env.obs_space['poi_state'].shape[0]
    agent_args.action_space = env.poi_action_space
    ...
    pi_args = Config()
    pi_args.sizes = [-1, 256, 128]  # 9是硬编码的离散动作数
    pi_args.branchs = env.poi_action_space['poi_uav'].nvec[0]
