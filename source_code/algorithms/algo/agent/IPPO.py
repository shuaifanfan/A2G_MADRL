...
class IPPOAgent(nn.ModuleList, AgentBase):
    ...
    def __init__(self, logger, device, agent_args, input_args):
        ...
        if self.use_mate:
            ...
            if self.share_mate:
                mate = MATE(
                    [Box(low=-1, high=1, shape=(self.observation_dim['uav'],)) for _ in range(self.n_agent['uav'])],
                    [self.action_space['uav'] for _ in range(self.n_agent['uav'])],
                    gcn_models=[self.gcn_model['uav'][i] for i in range(self.n_agent['uav'])], config_path=config_path,
                    config=agent_args)
            else:
                mate = None
            for type in self.agent_type:
                self.mate[type] = MATE(
                    [Box(low=-1, high=1, shape=(self.observation_dim[type],)) for _ in range(self.n_agent[type])],
                    [self.action_space[type] for _ in range(self.n_agent[type])],
                    gcn_models=[self.gcn_model[type][i] for i in range(self.n_agent['uav'])], config_path=config_path,
                    config=agent_args) if mate is None else mate
    ...
    def encode(self, s, a, r, ae_h, no_grads=True, type=None, graph_input=None):
        all_type = self.agent_type if type is None else [type]
        task_emb_all = {}
        ae_hiddens_all = {}
        for type in all_type:
            if len(all_type) == 1:
                s_, a_, r_, ae_h_ = [torch.as_tensor(item, dtype=torch.float32, device=self.device) for item in
                                     [s, a, r, ae_h]]
                if self.use_gcn:
                    nodes, edges = graph_input
                    nodes = nodes.permute(1, 0, 2, 3).contiguous()
                    edges = edges.permute(1, 0, 2, 3).contiguous()
                else:
                    nodes = [None for _ in range(self.n_agent[type])]
                    edges = [None for _ in range(self.n_agent[type])]
            else:
                if self.use_gcn:
                    nodes = torch.as_tensor(s['Nodes_' + type], dtype=torch.float32, device=self.device)
                    edges = torch.as_tensor(s['Edges_' + type], dtype=torch.float32, device=self.device)
                    nodes = nodes.permute(1, 0, 2, 3).contiguous()
                    edges = edges.permute(1, 0, 2, 3).contiguous()
                else:
                    nodes = [None for _ in range(self.n_agent[type])]
                    edges = [None for _ in range(self.n_agent[type])]
                s_, a_, r_, ae_h_ = [torch.as_tensor(item[type], dtype=torch.float32, device=self.device) for item in
                                     [s, a, r, ae_h]]
            ...
            task_emb, ae_hiddens = self.mate[type].encode(s_, a_, r_, ae_h_, no_grads, [nodes, edges])
    ...
    def update_encoder(self, trajs):
        ...
        for type in self.agent_type:
            ...
            if self.use_gcn:
                ...
                nodes = [nodes[:, :, i, :, :] for i in range(self.n_agent[type])]
                edges = [edges[:, :, i, :, :] for i in range(self.n_agent[type])]
            else:
                nodes = edges = None
            ...
            loss_dict = self.mate[type].update(s, a, ae_h, r, s1, d, [nodes, edges])
            ...
            for type in self.agent_type:
                ...
                for i in range(self.n_agent[type]):
                    ...
                    if task_emb is not None:
                        ...
                        probs.append(
                            self.actors[type][i](s[:, i, :], task_emb=t_emb[:, i, :], graph_inputs=g_in))  # TODO 形状
                    else:
    ...
    def updateAgent(self, trajs, clip=None):
        ...
        for type in self.agent_type:
            ...
            for i_update in range(1):
                ...
                if self.use_lambda:
                    value_old_cost, returns_cost, advantages_cost, _ = self._process_traj(traj['s'], traj['a'],
                                                                                          traj['cost'], traj['s1'],
                                                                                          traj['d'], traj['logp'],
                                                                                          traj.get('h', None),
                                                                                          traj.get('ae_h', None),
                ...
                for i_v in range(self.n_update_v):
                    ...
                    if self.use_lambda:
                        ...
                        log_dict = {'cost_loss': loss_cost, 'cost_update': None, 'cost_var': var_cost,
                                    'rel_cost_loss': rel_cost_loss, 'cost_update_step': i_cost}
                        self.logger.log(**{'loss/' + type + '_' + key: log_dict[key] for key in log_dict.keys()})
    ...
    def save_nets(self, dir_name, iter=0, is_newbest=False):
        ...
        torch.save(self.vs.state_dict(), dir_name + '/Models/' + prefix + '_critic.pt')
        if self.use_mate:
            for type in self.agent_type:
    ...
    def load_nets(self, dir_name, iter=0, best=False):
        ...
        self.vs.load_state_dict(torch.load(dir_name + '/Models/' + prefix + '_critic.pt'))
        if self.use_mate:
            for type in self.agent_type:
